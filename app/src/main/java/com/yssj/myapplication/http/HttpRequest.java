...
import com.yssj.myapplication.http.base.Base64Utils;
import com.yssj.myapplication.http.base.BaseOkHttp;
import com.yssj.myapplication.utils.JsonUtils;
...
public class HttpRequest extends BaseOkHttp {
    ...
    public static void POST(Context context, String url, Parameter parameter, BaseResponseListener listener) {
        if (TokenUtils.getToken() != null && TokenUtils.getToken().length()>0) {
            parameter.put("token", TokenUtils.getToken());
        }
    ...
    public static void POST(Context context, String url, Parameter headers, Parameter parameter, BaseResponseListener listener) {
        synchronized (HttpRequest.class) {
            ...
            String body = url + "?" + parameter.toParameterString();
            String authKey = null;
            try {
    ...
    public static void JSONPOST(Context context, String url, JsonMap jsonMap, BaseResponseListener listener) {
        if (TokenUtils.getToken() != null && TokenUtils.getToken().length()>0) {
            jsonMap.put("token", TokenUtils.getToken());
        }
        JSONPOST(context, url, null, JsonUtils.toJSONString(jsonMap), listener);
    }
    ...
    public static void JSONPUT(Context context, String url, JsonMap jsonMap, BaseResponseListener listener) {
        if (TokenUtils.getToken() != null && TokenUtils.getToken().length()>0) {
            jsonMap.put("token", TokenUtils.getToken());
        }
        JSONPUT(context, url, null, JsonUtils.toJSONString(jsonMap), listener);
    }

    //JSON格式POST一步创建总方法
    public static void JSONPUT(Context context, String url, Parameter headers, String jsonParameter, BaseResponseListener listener) {
        synchronized (HttpRequest.class) {
            HttpRequest httpRequest = new HttpRequest();
            httpRequest.context = new WeakReference<Context>(context);
            httpRequest.mContext = context;
...
//            url = md5_url(url);//生成签名和密钥
            ...
            if (isFileRequest) {
                ...
                if (parameterInterceptListener != null) {
                    try {
                        parameter = (Parameter) parameterInterceptListener.onIntercept(mContext, url, parameter);
                    } catch (Exception e) {
                    }
                }
            ...
            switch (requestType) {
                case GET_REQUEST:
                    //GET
                    if(parameter.toParameterString() != ""){
            ...
            if (TokenUtils.getToken() != null && TokenUtils.getToken().length()>0) {
                allHeader.put("token", TokenUtils.getToken());
            }
                                ...
                                if (responseInterceptListener != null) {
                                    if (responseInterceptListener.response(context.get(), url, null, e)) {
                                        if (responseListener != null) {
                                            responseListener.response(mContext,null, e);
                                        }
                                    }
                                } else {
                                    if (responseListener != null) {
                        ...
                        public void run() {
                            if (responseInterceptListener != null) {
                                if (responseInterceptListener.response(context.get(), url, strResponse, null)) {
                                    if (responseListener != null) {
                                        responseListener.response(mContext,strResponse, null);
                                    }
                                }
                            } else {
                                if (responseListener != null) {
    ...
    public String md5_url(String url){

            String oldurl = url + "yunshangshiji";
            String autherkey = MD5.getMessageDigest(oldurl.toString().getBytes())
                    .toUpperCase();;
            String I10o = Base64Utils.encode(autherkey.getBytes());
            String newurl = url + "&authKey=" + autherkey + "&I10o=" + I10o;
            return newurl;
    }
                    ...
                    if (BaseOkHttp.reserveServiceUrls != null && BaseOkHttp.reserveServiceUrls.length != 0) {
                        ...
                        runOnMain(new Runnable() {
                            ...
                            public void run() {
                                if (responseInterceptListener == null) {
                                    if (responseListener != null) {
                                        responseListener.response(mContext,null, new TimeOutException());
                                    }
                                } else {
